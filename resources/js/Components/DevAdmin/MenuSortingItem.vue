    ...
    <div class="menu-item">
        ...
        <div class="bg-white dark:bg-gray-800 p-1 rounded-lg border shadow-sm flex justify-between items-center hover:shadow-md transition-shadow"
            ...
            <div class="flex items-center space-x-3 flex-1">
                <button @click.prevent="isDragging = !isDragging"
                    ...
                    <span v-if="menu.icon" class="material-symbols-outlined"
                        :class="color === 'indigo' ? 'text-indigo-500' : 'text-emerald-500'">{{ menu.icon }}</span>
                </button>
                <span class="font-medium text-gray-800 dark:text-gray-200">{{ menu.name }}</span>
            </div>
            <div class="flex items-center space-x-2">
                <span
                    :class="['px-2 py-1 text-xs rounded', color === 'indigo' ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300' : 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300']">
                    Order: {{ menu.order }}
                </span>
                <span
                    :class="['px-2 py-1 text-xs rounded', menu.is_active ? (color === 'indigo' ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300' : 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300') : 'bg-rose-100 dark:bg-rose-900/30 text-rose-800 dark:text-rose-300']">
        ...
        <draggable v-if="localMenu.children && localMenu.children.length > 0" v-model="localMenu.children" item-key="id"
            :options="dragOptions" class="space-y-2 mt-2" @change="emitUpdate" tag="div">
            <template #item="{ element }">
                <MenuSortingItem :menu="element" :level="level + 1" :color="color" :route-prefix="routePrefix"
