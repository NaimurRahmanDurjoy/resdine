<template>
    <DevAdminLayout>
        ...
        <Head :title="isEdit ? 'Update Menu' : 'Add Menu'" />
            ...
            <div
                ...
                <div
                    ...
                    <div class="flex justify-between items-center">
                        <div>
                            <h1 class="text-xl font-bold text-gray-800 dark:text-white">{{ isEdit ? 'Update Menu' : 'Add
                                Menu' }}</h1>
                            <p class="text-gray-600 dark:text-gray-400 text-sm mt-1">{{ isEdit ? 'Update existing menu
                                item' : 'Create a new menu for your software' }}</p>
                        </div>
                        <Link :href="route('devAdmin.systemConfig.adminPanel.menu.index')"
                ...
                <form @submit.prevent="submit" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" v-model="form.name" required
                                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all shadow-sm"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" v-model="form.icon" placeholder="e.g. dashboard, settings"
                                    class="w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all shadow-sm">
                                <span
                        ...
                        <div>
                            ...
                            <select v-model="form.parent_id"
                                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all shadow-sm">
                                <option :value="null">None (Set as Parent)</option>
                    ...
                    <div class="pt-6 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-3">
                        ...
                        <button type="submit" :disabled="form.processing"
                            class="bg-indigo-600 text-white px-8 py-2 rounded-lg font-bold hover:bg-indigo-700 active:scale-95 transition-all shadow-lg shadow-indigo-500/20 disabled:opacity-50">
...
const submit = () => {
    if (props.isEdit) {
        form.put(route('devAdmin.systemConfig.adminPanel.menu.update', props.menu.id))
    } else {
        form.post(route('devAdmin.systemConfig.adminPanel.menu.store'))
    }
}
</script>
