<template>
  <div class="bg-gray-950 flex items-center justify-center min-h-screen soft-grid px-4 relative overflow-hidden">
    ...
    <!-- PREMIUM LOGIN CARD -->
    <div
      class="premium-card w-full max-w-md bg-gray-900/60 backdrop-blur-2xl border border-gray-700 rounded-2xl shadow-2xl p-10 relative z-20">
      <h2 class="text-center text-4xl font-extrabold text-cyan-400 drop-shadow-lg tracking-wide mb-8">
        Developer Admin
      </h2>

      <div v-if="$page.props.flash.error"
        class="mb-4 text-red-400 bg-red-900/30 px-3 py-2 rounded-md backdrop-blur text-sm">
      ...
      <form @submit.prevent="submit">
        <div class="mb-6">
          ...
          <input type="text" v-model="form.email" required class="w-full px-4 py-3 bg-gray-800/70 border border-gray-700 rounded-lg text-gray-200
        ...
        <div class="mb-8">
          ...
          <input type="password" v-model="form.password" required class="w-full px-4 py-3 bg-gray-800/70 border border-gray-700 rounded-lg text-gray-200
                   focus:ring-2 focus:ring-cyan-400 focus:border-transparent transition placeholder-gray-500"
            placeholder="••••••••">
        </div>

        <button :disabled="form.processing" class="w-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500
