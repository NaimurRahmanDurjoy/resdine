<template>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-white dark:bg-gray-800">
                <div class="bg-gradient-to-r from-indigo-50 to-white dark:from-gray-700 dark:to-gray-800 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">Manage table bookings and guest attendance</p>
                        </div>
                    </div>
                </div>
                ...
                <ListTable :headers="headers" :items="reservations.data" :pagination="reservations" :sort="filters.sort" :direction="filters.direction" :loading="loading" @sort="sortColumn">
                    <template #rows="{ items }">
                        <tr v-for="reservation in items" :key="reservation.id"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                ...
                                <span class="text-xs text-gray-500">{{ reservation.customer?.phone || reservation.customer_phone }}</span>
                            </td>
                    ...
                    <template #pagination>
                        ...
                        <div class="text-sm text-gray-700 dark:text-gray-400">
                            Showing <span class="font-medium">{{ reservations.from }}</span> to <span class="font-medium">{{ reservations.to
                            }}</span> of <span class="font-medium">{{ reservations.total }}</span> entries
                        </div>
                        <div class="flex space-x-1">
                            <Link v-for="(link, k) in reservations.links" :key="k" :href="link.url || '#'" v-html="link.label"
