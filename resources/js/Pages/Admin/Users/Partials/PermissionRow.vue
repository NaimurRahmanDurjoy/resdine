<template>
    <tr class="group hover:bg-gray-50/80 dark:hover:bg-gray-800/40 transition-colors border-l-2" :class="statusClass">
        <td class="py-2.5 px-4">
            <div class="flex items-center gap-3" :class="{ 'pl-8': indent }">
                <div class="flex flex-col">
                    <span class="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-widest">{{
                        action.action }}</span>
                    <code
                        class="text-[10px] text-gray-400 dark:text-gray-500 font-mono tracking-tighter">{{ action.route }}</code>
                </div>
                <!-- Status Badge -->
                <div class="flex items-center gap-2">
                    ...
                    <button v-if="!isInherited" @click="update(null)" title="Revert to Role Default"
                        class="text-gray-400 hover:text-rose-500 transition-colors flex items-center">
        ...
        <td class="py-2.5 px-4 text-right">
            <div
                class="inline-flex p-1 bg-gray-100 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-inner">
                <!-- Allow (True) -->
                <button @click="update(true)" type="button"
                    class="px-4 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all"
...
const override = computed(() => props.overrides.find(o => Number(o.action_id) === Number(props.action.id)))
const state = computed(() => override.value ? override.value.is_allowed : null)
const isInherited = computed(() => state.value === null)
const roleAllows = computed(() => props.rolePermissions.map(id => Number(id)).includes(Number(props.action.id)))
