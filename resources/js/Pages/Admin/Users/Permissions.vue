                ...
                <div class="bg-gradient-to-r from-blue-50 to-white dark:from-blue-900/20 dark:to-gray-800 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center gap-4">
                            <Link :href="route('admin.users.index')" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2 mt-0.5">
                                    <span class="text-xs text-gray-600 dark:text-gray-400">Overriding permissions for</span>
                                    <span class="text-xs font-bold text-blue-600 dark:text-blue-400 underline decoration-blue-500/30 whitespace-nowrap">{{ user.name }}</span>
                                    <span class="px-2 py-0.25 bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 text-[9px] font-bold rounded border border-blue-100 dark:border-blue-800 uppercase tracking-tighter">
                                        Role: {{ user.role.name }}
                                    </span>
                                </div>
                            </div>
                ...
                <div class="p-6 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30 space-y-4">
                    <div class="flex flex-wrap gap-6 justify-between items-center">
                        <div class="flex items-center gap-8">
                             <div class="flex items-center gap-2" title="Permission matches Role default">
                                <div class="w-3.5 h-3.5 rounded-md bg-white border border-gray-300 dark:bg-gray-700"></div>
                                <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Inherit</span>
                            </div>
                            <div class="flex items-center gap-2" title="User is explicitly granted access">
                                ...
                                <span class="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Explicit Allow</span>
                            </div>
                            <div class="flex items-center gap-2" title="User is explicitly denied access">
                                ...
                                <span class="text-[10px] font-bold text-rose-600 uppercase tracking-widest">Explicit Deny</span>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center gap-6 pt-2">
                        <div class="flex items-center gap-3">
                            <span class="text-[10px] font-black uppercase tracking-tighter text-gray-400 whitespace-nowrap">Global Actions:</span>
                            <div class="flex bg-white dark:bg-gray-800 p-0.5 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                                <button @click="bulkApply(null)" type="button" class="px-3 py-1 text-[9px] font-bold uppercase tracking-widest text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-900/20 rounded-md transition-colors">Inherit All</button>
                    ...
                    <table class="w-full border-collapse">
                        <thead>
                            <tr class="text-left border-b border-gray-100 dark:border-gray-700">
                                <th class="pb-3 text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">Menu / Action Hierarchy</th>
                                <th class="pb-3 text-right text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">Override State (Inherit/Allow/Deny)</th>
                            </tr>
                        </thead>
                            ...
                            <template v-for="menu in filteredMenus" :key="menu.id">
                                ...
                                <tr class="bg-gray-50/50 dark:bg-gray-900/20 group cursor-pointer" @click="toggleMenu(menu.id)">
                                    <td class="py-3 px-2">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <span class="material-symbols-outlined text-gray-300 dark:text-gray-600 text-xl font-icon">{{ menu.icon || 'folder' }}</span>
                                    ...
                                    <td class="py-1 px-4 text-right">
                                        <div class="inline-flex gap-1 items-center bg-white dark:bg-gray-800 p-1 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity" @click.stop>
                                            ...
                                            <button @click="bulkApplyToMenu(menu, null)" type="button" title="Set to Inherit for all sub-items" class="p-1 hover:bg-gray-50 dark:hover:bg-gray-900/20 rounded text-gray-400 material-symbols-outlined text-sm">settings_backup_restore</button>
                                            <button @click="bulkApplyToMenu(menu, true)" type="button" title="Grant All under this Menu" class="p-1 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded text-emerald-500 material-symbols-outlined text-sm">check_circle</button>
                                            <button @click="bulkApplyToMenu(menu, false)" type="button" title="Revoke All under this Menu" class="p-1 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded text-rose-500 material-symbols-outlined text-sm">cancel</button>
                                        </div>
                                    </td>
                                </tr>
                                <!-- Actions & Children -->
                                <template v-if="isExpanded(menu.id)">
                                    <template v-for="action in menu.actions" :key="action.id">
                                        <PermissionRow :action="action" :overrides="form.overrides" :rolePermissions="rolePermissions" @update="handleUpdate" />
                                    </template>
                                    <template v-for="child in menu.children_recursive" :key="child.id">
                                        <tr class="bg-gray-50/20 dark:bg-gray-900/10 border-l-4 border-gray-100 dark:border-gray-800 group cursor-pointer" @click="toggleMenu(child.id)">
                                            <td class="py-3 px-4">
                                                <div class="flex items-center space-x-3 pl-4">
                                                    <span class="material-symbols-outlined text-gray-400 dark:text-gray-500 text-sm font-icon transition-transform" :class="{ 'rotate-90 text-blue-500': isExpanded(child.id) }">chevron_right</span>
                                                    <span class="material-symbols-outlined text-gray-400 dark:text-gray-600 text-lg font-icon">{{ child.icon || 'subdirectory_arrow_right' }}</span>
                                                    <span class="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-widest">{{ child.name }}</span>
                                                </div>
                                            </td>
                                            <td class="py-1 px-4 text-right">
                                                <div class="inline-flex gap-1 items-center bg-white dark:bg-gray-800 p-0.5 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity" @click.stop>
                                                    <button @click="bulkApplyToMenu(child, null)" type="button" class="p-1 hover:bg-gray-50 dark:hover:bg-gray-900/20 rounded text-gray-400 material-symbols-outlined text-xs">settings_backup_restore</button>
                                        ...
                                        <template v-if="isExpanded(child.id)">
                                            <template v-for="childAction in child.actions" :key="childAction.id">
                                                <PermissionRow :action="childAction" :overrides="form.overrides" :rolePermissions="rolePermissions" :indent="true" @update="handleUpdate" />
                                            </template>
                                        </template>
                                    </template>
                                </template>
