        ...
        <tr v-for="user in items" :key="user.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div class="flex justify-end space-x-2">
              <Link :href="`/admin/users/${user.id}/edit`" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
...
const performSearch = () => {
  router.get('/admin/users', { search: search.value, sort: props.filters.sort, direction: props.filters.direction }, {
    preserveState: true,
    replace: true
  })
}
...
const sortBy = (field) => {
  ...
  router.get('/admin/users', { search: search.value, sort: field, direction }, {
    preserveState: true,
    replace: true
  })
}
