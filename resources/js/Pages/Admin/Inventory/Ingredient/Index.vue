<template>
    <AdminLayout :pageTitle="pageTitle">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <!-- Content -->
            <ListTable :headers="headers" :items="ingredients.data" :pagination="ingredients" :sort="filters.sort"
                    ...
                    <tr v-for="ingredient in items" :key="ingredient.id"
                        ...
                        <td class="px-6 py-2 whitespace-nowrap">
                            <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full"
                                :class="ingredient.has_expiry ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-800'">
                                {{ ingredient.has_expiry ? 'Yes' : 'No' }}
                            </span>
                        </td>
                        <td class="px-6 py-2 whitespace-nowrap">
                            <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full"
                ...
                <div class="flex items-center justify-between w-full">
                    <div class="text-sm text-gray-700 dark:text-gray-400">
                        Showing <span class="font-medium">{{ ingredients.from }}</span> to <span class="font-medium">{{
                            ingredients.to
                            }}</span> of <span class="font-medium">{{ ingredients.total }}</span> entries
                    </div>
                    <div class="flex space-x-1">
                        <Link v-for="(link, k) in ingredients.links" :key="k" :href="link.url || '#'" v-html="link.label"
...
const headers = [
    ...
    { label: 'Unit Name', key: 'unit_name', sortable: true },
    { label: 'Min Stock', key: 'min_stock', sortable: true },
    { label: 'Expiry Tracking', key: 'expiry_tracking', sortable: true },
...
const performSearch = () => {
    loading.value = true
    router.get(route('admin.ingredients.index'), { search: search.value, sort: props.filters.sort, direction: props.filters.direction }, {
        preserveState: true,
        replace: true,
...
function sortColumn(column) {
    ...
    loading.value = true
    router.get(route('admin.ingredients.index'), { search: search.value, sort: column, direction: direction }, {
        preserveState: true,
        replace: true,
        onFinish: () => loading.value = false
    })
