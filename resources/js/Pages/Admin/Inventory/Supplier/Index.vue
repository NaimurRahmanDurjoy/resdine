<template>
    <AdminLayout :pageTitle="pageTitle">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="bg-white dark:bg-gray-800">
                ...
                <div class="m-4 flex justify-end items-center">
                    ...
                    <button @click="performSearch"
                        class="ml-2 px-3 py-1 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700 transition">
                        Search
                    </button>
                </div>
                <!-- Add new item -->
            ...
            <!-- Content -->
             <ListTable :headers="headers" :items="suppliers.data" :pagination="suppliers" :sort="filters.sort" :direction="filters.direction" :loading="loading" @sort="sortColumn">
                    <template #rows="{ items }">
...
const performSearch = () => {
    loading.value = true
    router.get(route('admin.suppliers.index'), { search: search.value, sort: props.filters.sort, direction: props.filters.direction }, {
        preserveState: true,
        replace: true,
...
function sortColumn(column) {
    ...
    loading.value = true
    router.get(route('admin.suppliers.index'), { search: search.value, sort: column, direction: direction }, {
        preserveState: true,
        replace: true,
        onFinish: () => loading.value = false
    })
