...
const TRANSACTION_TYPES = {
    ...
    9: { label: 'Production', class: 'bg-cyan-100 text-cyan-700 border-cyan-200' },
}
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                    ...
                    <div class="text-3xl font-bold text-indigo-600">
                        {{ item.inventory_item?.stock_summary ?
                            parseFloat(item.inventory_item.stock_summary.current_stock).toFixed(2) :
                            '0.00'
                        }}
                        <span class="text-sm font-normal text-gray-400">{{ item.unit?.name ||
                            item.unit?.short_name }}</span>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                    ...
                    <div class="text-3xl font-bold text-gray-900">
                        {{ parseFloat(item.inventory_item?.min_stock || 0).toFixed(2) }}
                        <span class="text-sm font-normal text-gray-400">{{ item.unit?.name ||
                            item.unit?.short_name }}</span>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                    ...
                    <div class="flex items-center gap-2 mt-2">
                        <span
                            v-if="item.inventory_item?.stock_summary && parseFloat(item.inventory_item.stock_summary.current_stock) <= parseFloat(item.inventory_item.min_stock)"
                            class="px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200">
                            Low Stock Alert
                        </span>
                        <span
                            v-else-if="!item.inventory_item?.stock_summary && parseFloat(item.inventory_item?.min_stock || 0) > 0"
