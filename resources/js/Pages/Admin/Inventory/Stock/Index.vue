                ...
                <template #rows="{ items }">
                    <tr v-for="stock in items" :key="stock.id"
                        ...
                        <td class="px-6 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-l-4 border-transparent"
                            ...
                            <span v-else-if="stock.stock_summary && parseFloat(stock.stock_summary.current_stock) <= parseFloat(stock.min_stock)"
                        ...
                        <td class="px-6 py-2 whitespace-nowrap text-sm">
                            <div v-if="stock.purchase_details && stock.purchase_details.length > 0" class="flex flex-col gap-1">
                        ...
                        <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-500 font-medium text-right font-mono">
                             {{ stock.stock_summary?.updated_at ? new Date(stock.stock_summary.updated_at).toLocaleDateString() : 'N/A' }}
                        </td>
                        <td class="px-6 py-2 whitespace-nowrap text-sm text-center">
                            <Link :href="route('admin.stock.show', stock.id)" 
                                ...
                                title="View History">
                                <span class="material-symbols-outlined text-sm group-hover:scale-110 transition-transform">manage_search</span>
                            </Link>
                        </td>
                    </tr>
                </template>
