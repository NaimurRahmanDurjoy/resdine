            ...
            <div class="p-8">
                <div class="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-3">
                    ...
                    <p class="text-sm text-yellow-800">
                        <strong>Note:</strong> Standard intakes should be recorded as Purchase Orders. Use this
                        adjustment tool only to correct discrepancies, account for spoilage, or log non-purchased
                        intakes.
                    </p>
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="space-y-5">
                        ...
                        <div v-if="form.item_id"
                            ...
                            <div>
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ currentStockDisplay }} <span
                                        class="text-sm text-gray-500 font-normal">{{ unitDisplay }}</span></p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Transaction
                                    Type *</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Quantity
                                    to Adjust *</label>
                                <div class="relative">
                                    <input v-model.number="form.quantity" type="number" step="0.01" min="0.01"
                                        placeholder="0.00"
                                        ...
                                        required />
                                    <div
                                        class="absolute inset-y-0 right-0 py-2 pr-3 flex items-center pointer-events-none">
                                        <span class="text-gray-500 text-sm font-medium">{{ unitDisplay }}</span>
                                    </div>
                                </div>
                                <p v-if="form.transaction_type === 'out' && (form.quantity > currentStockValue)"
                    ...
                    <div class="pt-6 border-t border-gray-100 dark:border-gray-700">
                        <div class="flex items-center gap-3">
                            <button type="submit"
                                :disabled="form.transaction_type === 'out' && form.quantity > currentStockValue"
