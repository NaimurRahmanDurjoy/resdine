  ...
  <div class="theme-panel overflow-hidden max-w-4xl mx-auto">
    ...
    <div
      ...
      <div>
        ...
        <p class="text-gray-600 dark:text-gray-400 text-sm mt-1">Define membership tiers and benefits</p>
      </div>
    ...
    <form @submit.prevent="submit" class="p-6 space-y-4">
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Tier Name</label>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input v-model="form.discount_percentage" type="number" step="0.01"
            class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
            required>
          <div v-if="form.errors.discount_percentage" class="text-red-500 text-xs mt-1">{{
            form.errors.discount_percentage }}</div>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Loyalty Multiplier</label>
          <input v-model="form.loyalty_multiplier" type="number"
            class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
            required>
          <div v-if="form.errors.loyalty_multiplier" class="text-red-500 text-xs mt-1">{{ form.errors.loyalty_multiplier
          }}</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Min Points</label>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Max Points (Optional)</label>
