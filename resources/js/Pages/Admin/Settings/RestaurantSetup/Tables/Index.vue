<template>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <!-- Content -->
                ...
                <ListTable :headers="headers" :items="tables.data" :pagination="tables" :sort="filters.sort" :direction="filters.direction" :loading="loading" @sort="sortColumn">
                    <template #rows="{ items }">
                    ...
                    <template #pagination>
                        ...
                        <div class="text-sm text-gray-700 dark:text-gray-400">
                            Showing <span class="font-medium">{{ tables.from }}</span> to <span class="font-medium">{{ tables.to }}</span> of <span class="font-medium">{{ tables.total }}</span> entries
                        </div>
                        <div class="flex space-x-1">
                            <Link v-for="(link, k) in tables.links" :key="k" :href="link.url || '#'" v-html="link.label"
...
const performSearch = () => {
  loading.value = true
  router.get(route('admin.settings.restaurant-setup.tables.index'), { search: search.value, sort: props.filters.sort, direction: props.filters.direction }, {
    preserveState: true,
    replace: true,
...
function sortColumn(column){
  ...
  loading.value = true
  router.get(route('admin.settings.restaurant-setup.tables.index'), { search: search.value, sort: column, direction: direction }, {
    preserveState: true,
    replace: true,
    onFinish: () => loading.value = false
  })
