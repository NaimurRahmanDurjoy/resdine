    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="bg-white dark:bg-gray-800">
            <div
                ...
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">Manage your restaurant branches and locations</p>
                    </div>
                </div>
            </div>
            <div class="m-4 flex justify-between items-center">
                ...
                <!-- Add new item -->
                <Link :href="route('admin.settings.restaurant-setup.branches.create')"
        ...
        <!-- Content -->
        <ListTable :headers="headers" :items="branches.data" :pagination="branches" :sort="filters.sort"
                ...
                <tr v-for="branch in items" :key="branch.id"
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full"
                            :class="branch.status ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'">
                            {{ branch.status ? 'Active' : 'Inactive' }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex gap-3">
                            <Link :href="route('admin.settings.restaurant-setup.branches.edit', branch.id)"
            ...
            <template #pagination>
                <div class="flex items-center justify-between w-full">
                    <div class="text-sm text-gray-700 dark:text-gray-400">
                        Showing <span class="font-medium">{{ branches.from }}</span> to <span class="font-medium">
                            {{ branches.to }}</span> of <span class="font-medium">{{ branches.total }}</span> entries
                    </div>
                    <div class="flex space-x-1">
                        <Link v-for="(link, k) in branches.links" :key="k" :href="link.url || '#'" v-html="link.label"
...
const performSearch = () => {
  loading.value = true
  router.get(route('admin.settings.restaurant-setup.branches.index'), { search: search.value, sort: props.filters.sort, direction: props.filters.direction }, {
    preserveState: true,
    replace: true,
...
function sortColumn(column){
  ...
  loading.value = true
  router.get(route('admin.settings.restaurant-setup.branches.index'), { search: search.value, sort: column, direction: direction }, {
    preserveState: true,
    replace: true,
    onFinish: () => loading.value = false
  })
...
const deleteBranch = (id) => {
    Swal.fire({
        title: 'Are you sure?',
        text: "Deleting a branch may affect related data like tables and departments!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            router.delete(route('admin.settings.restaurant-setup.branches.destroy', id), {
                onSuccess: () => {
