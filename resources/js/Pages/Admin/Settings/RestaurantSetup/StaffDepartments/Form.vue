    ...
    <form @submit.prevent="submit" class="space-y-6">
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="flex items-start gap-6">
                ...
                <div class="flex-1">
                    <input v-model="form.name" type="text" placeholder="E.g. Management, Service, Cleaning"
                        class="w-full h-10 border rounded-lg border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-200 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition px-3 text-sm"
                        required />
...
const submit = () => {
    if (props.isEdit) {
        form.put(route('admin.settings.restaurant-setup.staff-departments.update', props.department.id), {
            preserveScroll: true
        })
    } else {
        form.post(route('admin.settings.restaurant-setup.staff-departments.store'), {
            preserveScroll: true
        })
    }
}
