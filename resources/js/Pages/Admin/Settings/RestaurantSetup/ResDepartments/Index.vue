<template>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="bg-white dark:bg-gray-800">
            <div
                ...
                <div class="flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold text-gray-800 dark:text-gray-100">Restaurant Departments</h1>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Manage kitchen, bar, bakery, and other
                            service areas</p>
                    </div>
                </div>
            </div>
            <div class="m-4 flex justify-between items-center">
                ...
                <!-- Add new item -->
                <Link :href="route('admin.settings.restaurant-setup.res-departments.create')"
        ...
        <!-- Content -->
        <ListTable :headers="headers" :items="departments.data" :pagination="departments" :sort="filters.sort"
                ...
                <tr v-for="dept in items" :key="dept.id"
                    ...
                    <td class="px-6 py-2 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex gap-3">
                            <Link :href="route('admin.settings.restaurant-setup.res-departments.edit', dept.id)"
            ...
            <template #pagination>
                <div class="flex items-center justify-between w-full">
                    <div class="text-sm text-gray-700 dark:text-gray-400">
                        Showing <span class="font-medium">{{ departments.from }}</span> to <span class="font-medium">{{
                            departments.to }}</span> of <span class="font-medium">{{ departments.total }}</span> entries
                    </div>
                    <div class="flex space-x-1">
                        <Link v-for="(link, k) in departments.links" :key="k" :href="link.url || '#'"
                            v-html="link.label"
...
const performSearch = () => {
    loading.value = true
    router.get(route('admin.settings.restaurant-setup.res-departments.index'), { search: search.value, sort: props.filters.sort, direction: props.filters.direction }, {
        preserveState: true,
        replace: true,
...
function sortColumn(column) {
    ...
    loading.value = true
    router.get(route('admin.settings.restaurant-setup.res-departments.index'), { search: search.value, sort: column, direction: direction }, {
        preserveState: true,
        replace: true,
        onFinish: () => loading.value = false
    })
...
const deleteDept = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('admin.settings.restaurant-setup.res-departments.destroy', id), {
                onSuccess: () => {
