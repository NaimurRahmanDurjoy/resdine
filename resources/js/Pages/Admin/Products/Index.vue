<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <!-- Products Table -->
    <ListTable :headers="headers" :items="products.data" :pagination="products" :sort="filters.sort"
        ...
        <tr v-for="product in items" :key="product.id"
          ...
          <td class="px-6 py-2 whitespace-nowrap">
            <div class="relative inline-block">
              <Link :href="product.recipes_count === 0
                ? route('admin.recipes.create', { product_id: product.id })
                : (product.recipes_count === 1
                  ? route('admin.recipes.edit', product.recipe.id)
                  : route('admin.recipes.index', { menu_item_id: product.id }))"
                class="inline-flex items-center gap-1 px-3 py-1 rounded text-xs font-semibold transition-all shadow-sm"
                :class="product.recipes_count > 0
              ...
              <span v-if="product.recipes_count > 0"
                class="absolute -top-2 -right-2 bg-gray-700 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow">
