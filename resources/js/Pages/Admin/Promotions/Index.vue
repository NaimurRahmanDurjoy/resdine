<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <ListTable :headers="headers" :items="promotions.data" :pagination="promotions" :sort="filters.sort"
      ...
      <template #rows="{ items }">
        <tr v-for="promotion in items" :key="promotion.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="text-xs text-gray-500 dark:text-gray-400">
              {{ promotion.start_date }} to {{ promotion.end_date || 'Ongoing' }}
            </div>
          </td>
          <td class="px-6 py-4 whitespace-nowrap">
            <span
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div class="flex space-x-2">
              <Link :href="route('admin.promotions.edit', promotion.id)" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                <span class="material-symbols-outlined">edit</span>
              </Link>
              <button @click="deletePromotion(promotion.id)" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">
      ...
      <template #pagination>
        <div class="flex items-center justify-between w-full">
          <div class="text-sm text-gray-700 dark:text-gray-400">
            Showing <span class="font-medium">{{ promotions.from }}</span> to <span class="font-medium">{{ promotions.to }}</span> of <span class="font-medium">{{ promotions.total }}</span> entries
          </div>
          <div class="flex space-x-1">
            <Link
              v-for="(link, k) in promotions.links"
              :key="k"
              :href="link.url || '#'"
              v-html="link.label"
