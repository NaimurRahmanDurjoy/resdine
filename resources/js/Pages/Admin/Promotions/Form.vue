        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="col-span-2">
            ...
            <input v-model="form.name" type="text"
              class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white"
          ...
          <div class="col-span-2">
            <label class="flex items-center space-x-3 cursor-pointer">
              <input v-model="form.status" type="checkbox" :value="1" :true-value="1" :false-value="0"
          ...
          <!-- Applicable Items -->
          <div class="col-span-2">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Applicable Menu Items (Select
              multiple)</label>
            <div
              class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 max-h-60 overflow-y-auto p-4 border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-800/50">
              <label v-for="item in productItems" :key="item.id"
                class="flex items-center space-x-2 text-sm p-1 hover:bg-white dark:hover:bg-gray-700 rounded transition cursor-pointer">
                <input type="checkbox" :value="item.id" v-model="form.items"
                  class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                <span class="text-gray-700 dark:text-gray-300 truncate">{{ item.name }}</span>
              </label>
            </div>
            <p class="text-xs text-gray-500 mt-2 italic">If no items are selected, the promotion may apply to all
              (depending on your business logic implementation).</p>
          </div>
        </div>

        <div class="flex justify-end pt-4 border-t border-gray-100 dark:border-gray-700">
          <button type="submit" :disabled="form.processing"
            class="px-6 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition disabled:opacity-50">
