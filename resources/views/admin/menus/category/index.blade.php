...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="m-4 flex justify-between items-center">
        <form method="GET" action="{{ route('admin.product.categories.index') }}" class="flex space-x-2">
            <input
                type="text"
                name="search"
        ...
        <a href="{{ route('admin.product.categories.create') }}" class="px-3 py-1 bg-indigo-600 text-white rounded text-sm hover:bg-green-700 flex items-center space-x-2">
            ...
            <tr class="hover:bg-indigo-50 transition">
                ...
                <td class="px-6 py-1 whitespace-nowrap">
                    <div class="flex items-center space-x-4 text-base text-gray-600">
                        <!-- Edit -->
                        <a href="{{ route('admin.product.categories.edit', $category->id) }}" title="Edit" class="hover:text-indigo-600 transition">
                            <span class="material-symbols-outlined text-indigo-600" style="font-size: 20px;">edit</span>
                        </a>
                        <!-- Delete -->
                        <form action="{{ route('admin.product.categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Are you sure to want delete?')">
                            @csrf
                            @method('DELETE')
