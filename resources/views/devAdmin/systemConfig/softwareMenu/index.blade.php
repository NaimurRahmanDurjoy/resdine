...
<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">Menus</h1>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse ($menus as $menu)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-4 py-2 text-sm text-gray-800 dark:text-gray-200 font-medium">
                            {{ $menu->name }}
                            @if($menu->children->count())
                                <div class="pl-4 text-gray-500 dark:text-gray-400 text-xs">
                        ...
                        <td class="px-4 py-2 text-center">
                            ...
                            <form action="{{ route($baseRoute .'.destroy', $menu->id) }}" method="POST"
                                  class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Are you sure?')"
                                        class="text-red-600 hover:text-red-800 mx-1">Delete</button>
                            </form>
                        </td>
                    </tr>
                @empty
