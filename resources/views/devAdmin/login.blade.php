...
<body class="bg-gray-950 flex items-center justify-center min-h-screen soft-grid px-4">
    ...
    <div class="premium-card w-full max-w-md bg-gray-900/60 backdrop-blur-2xl border border-gray-700 rounded-2xl shadow-2xl p-10 relative z-20">
        ...
        @if(session('error'))
            <div class="mb-4 text-red-400 bg-red-900/30 px-3 py-2 rounded-md backdrop-blur">
                {{ session('error') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('devAdmin.login.submit') }}">
            ...
            <button class="w-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500
